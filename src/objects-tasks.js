...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    const countA = a.country;
    const countB = b.country;
    if (countA < countB) {
      return -1;
    }
    if (countA > countB) {
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    ...
    if (res.has(key)) {
      const valuesArr = res.get(key);
      res.set(key, [...valuesArr, value]);
